...
**1. Open VCV Rack and add the Audio module.**
From the module browser (right-click the empty rack), search for "Audio". Add the Audio-8 module (or Audio-2 if you prefer a simpler layout). In the module's device selector at the bottom, choose your audio interface — or your built-in sound card if that's what you have. If you're using a microphone plugged into your interface, make sure the correct input channel is active.

**2. Connect Audio OUT to Simpliciter IN.**
Drag a cable from the Audio module's first output jack (labelled OUT 1, on the left side of the Audio module) to Simpliciter's IN L jack. You're routing the live audio signal into the sampler's left channel. If your microphone signal comes through both channels, connect OUT 2 to IN R as well — but for now, mono is fine.

**3. Set Simpliciter to Record mode.**
Simpliciter has several operating modes. Look for the REC button — it's large and usually red. You don't press it to start recording yet; first, just press it once to arm the module. The display will show that it's ready. You should see the input level meter responding to sound. Speak, clap, or sing and confirm you're getting signal. If the meter doesn't move, check your cable routing and your interface settings.

**4. Press REC, make your sound, press REC again to stop.**
Now press REC a second time — this begins the actual recording. Make your sound. Press REC once more to stop. Simpliciter has now captured what it heard. The waveform display will show your recording as a shape — peaks where it was loud, flat where it was quiet.

**5. Press PLAY to hear it back.**
Press the PLAY button. Your sound plays back through Simpliciter's OUT jacks. Route those through a VCA or directly to your speakers (via the Audio module's IN jacks) to hear it. If it sounds like what you recorded, you've mastered Technique A. That's it.

One important practical note: Simpliciter saves recordings as `.wav` files in the same folder as your patch. If you save your patch to a dedicated project folder before you start recording, your audio files will live there too, neatly organised. If you don't save the patch first, Simpliciter will put its files in a temporary location that can be hard to find later. Save first. It's a good habit.
...
::: {.creative-option}
**Tape Saturation**: Real tape recorders saturated when recorded too hot — a warm, slightly compressed sound that many producers still chase today with expensive plugins. In VCV Rack, you can get something similar by patching your audio through NYSTHI Console (or any saturator module) before it reaches Simpliciter's IN jack. Push the input gain just past the sweet spot — you'll hear the transients soften and the body of the sound fill out. It doesn't sound broken. It sounds *warm*. Whether you want that warmth on a given recording is a creative decision, but it's worth knowing it's there.
:::
...
**Close**: the microphone is very near the sound source — a few centimetres. You hear the detail, the breath, the texture. Loud sounds can overload the input at this distance.
...
Record the same sound — your voice saying a single word, or a single hand clap — four times in a row, as four separate Simpliciter recordings: Close soft, Close loud, Reverb soft, Reverb loud. Play them back in sequence. The differences in character should be striking even with the same source material.
...
Dwyer was enthusiastic about reversal because it could transform utterly familiar sounds into the unfamiliar. The same is true here. The microwave you recorded an hour ago becomes, reversed, something you have never heard before.
